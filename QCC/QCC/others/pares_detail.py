...
__author__: lazy
@file: 
@time: 2019/4/19 16:06
@func:
"""
html = '''
D:\python\python.exe D:/BaiWang/爬虫五期/抬头/QCC/QCC/others/test.py
<!DOCTYPE html>
<html lang="zh-cn">
...
<body data-spm="7663354">
    ...
    <div class="header">
      <div class="container">
        <div class="message">
          ...
          <div>您的请求ID是：<strong>7c730acd15556627708715763e</strong></div>
        </div>
      </div>
    </div>
    <div class="main">
  ...
  <script type="text/javascript" charset="utf-8" src="https://errors.aliyun.com/error.js?s=3"></script>
</body>
...
print(name)
try:
    creditcode = re.findall(r'<td class="tb">统一社会信用代码</td> <td class="">(.*?)</td>',html, re.S | re.I | re.M)[0].strip()
except:
    creditcode = re.findall(r'<td class="tb"> 工商登记 </td> <td>(.*?)</td>', html, re.S | re.I | re.M)[0].strip()
print(creditcode)
try:
    adder = re.findall(r'<td class="tb">企业地址</td> <td class="" colspan="3">(.*?)<a',html, re.S | re.I | re.M)[0].strip()
except:
    adder = ''
print(adder)